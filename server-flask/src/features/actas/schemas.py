...
from src.models.enums import EquipmentType
...
def validate_acta_payload(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if not equipos or not isinstance(equipos, list) or len(equipos) == 0:
        raise ValidationError(
            "El campo 'equipos' es requerido y debe ser una lista no vacía."
        )
    ...
    for i, eq in enumerate(equipos):
        ...
        if eq["quantity"] <= 0:
            raise ValidationError(
                f"La cantidad debe ser un número positivo para el equipo #{i + 1}."
            )
        ...
        if type == EquipmentType.LAPTOP.value:
            if not eq["serial_number"] or eq["serial_number"].strip() == "":
                raise ValidationError(
                    f"Una Laptop DEBE tener número de serie (equipo #{i + 1})."
                )
            if not eq["model"] or eq["model"].strip() == "":
                raise ValidationError(
                    f"Una Laptop DEBE tener modelo (equipo #{i + 1})."
                )
            if "hostname" not in eq or eq["hostname"].strip() == "":
                raise ValidationError(
                    f"Una Laptop DEBE tener un Hostname asignado (equipo #{i + 1})."
                )

        elif type in [EquipmentType.CARGADOR.value, EquipmentType.DIADEMA.value]:
            if not eq["model"]:
                raise ValidationError(
                    f"El accesorio {type} requiere un modelo (equipo #{i + 1})."
                )
            if not eq["serial_number"] or eq["serial_number"].strip() == "":
                eq["serial_number"] = "NA"
