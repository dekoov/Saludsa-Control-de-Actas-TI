"""Funciones de formateo para fechas, montos y ubicaciones.

Este módulo centraliza las transformaciones de datos comunes usadas en la
generación de documentos y respuestas del sistema, como la conversión de
fechas a texto narrativo o legal, de montos numéricos a letras y de códigos
de oficina a nombres completos de ciudades.
"""

from datetime import datetime

from num2words import num2words

MONTHS_ES = [
    "enero",
    "febrero",
    "marzo",
    "abril",
    "mayo",
    "junio",
    "julio",
    "agosto",
    "septiembre",
    "octubre",
    "noviembre",
    "diciembre",
]
"""list[str]: Nombres de los meses en español en minúsculas."""

DAYS_ES = [
    "lunes",
    "martes",
    "miércoles",
    "jueves",
    "viernes",
    "sábado",
    "domingo",
]
"""list[str]: Nombres de los días de la semana en español en minúsculas."""

# Diccionario de mapeo de ciudades
CITY_MAP = {
    "GYE": "Guayaquil",
    "CUE": "Cuenca",
    "UIO": "Quito",
    "MAC": "Machala",
    "MAN": "Manta",
}
"""dict[str, str]: Mapeo de códigos de oficina a nombres completos de ciudades."""


def fecha_a_texto_extenso(fecha: datetime | None = None) -> str:
    """Convierte una fecha a formato narrativo completo.

    Args:
        fecha: Fecha a formatear. Si es ``None``, se utiliza la fecha y hora
            actual del sistema.

    Returns:
        Cadena con el formato ``'lunes, 16 de junio del 2026'``.

    Example:
        >>> from datetime import date
        >>> fecha_a_texto_extenso(date(2026, 6, 16))
        'martes, 16 de junio del 2026'
    """
    if fecha is None:
        fecha = datetime.now()

    # fecha.weekday() devuelve un entero de 0 (lunes) a 6 (domingo)
    dia_semana = DAYS_ES[fecha.weekday()]
    mes_minusculas = MONTHS_ES[fecha.month - 1]

    return f"{dia_semana}, {fecha.day} de {mes_minusculas} del {fecha.year}"


def fecha_a_texto_legal(fecha: datetime | None = None) -> str:
    """Convierte una fecha al formato legal en mayúsculas.

    Args:
        fecha: Fecha a formatear. Si es ``None``, se utiliza la fecha y hora
            actual del sistema.

    Returns:
        Cadena con el formato ``'14, ABRIL, 2026'``.

    Example:
        >>> from datetime import date
        >>> fecha_a_texto_legal(date(2026, 4, 14))
        '14, ABRIL, 2026'
    """
    if fecha is None:
        fecha = datetime.now()

    # 1. Buscamos el mes en tu lista (sale en minúsculas)
    mes_minusculas = MONTHS_ES[fecha.month - 1]

    # 2. Lo transformamos a MAYÚSCULAS en el return usando .upper()
    return f"{fecha.day}, {mes_minusculas.upper()}, {fecha.year}"


def ubicacion_a_letras(codigo_ad: str | None) -> str:
    """Convierte códigos de oficina a nombres completos de ciudad.

    Args:
        codigo_ad: Código de oficina, por ejemplo ``'GYE'`` o ``'uio'``.

    Returns:
        Nombre completo de la ciudad si el código existe en ``CITY_MAP``;
        de lo contrario, retorna el código normalizado a mayúsculas.
        Si el código está vacío, retorna ``'Ubicación no especificada'``.

    Example:
        >>> ubicacion_a_letras("GYE")
        'Guayaquil'
        >>> ubicacion_a_letras("XYZ")
        'XYZ'
    """
    if not codigo_ad:
        return "Ubicación no especificada"

    # .upper() por si acaso el AD lo devuelve en minúsculas
    codigo = codigo_ad.upper().strip()

    return CITY_MAP.get(codigo, codigo)


def fecha_a_texto(fecha: datetime | None = None) -> str:
    """Convierte una fecha a formato narrativo corto.

    Args:
        fecha: Fecha a formatear. Si es ``None``, se utiliza la fecha y hora
            actual del sistema.

    Returns:
        Cadena con el formato ``'13 de Abril del 2026'``.

    Example:
        >>> from datetime import date
        >>> fecha_a_texto(date(2026, 4, 13))
        '13 de Abril del 2026'
    """
    if fecha is None:
        fecha = datetime.now()

    return f"{fecha.day} de {MONTHS_ES[fecha.month - 1].capitalize()} del {fecha.year}"


def monto_a_letras(monto: float | int | str, incluir_centavos: bool = True) -> str:
    """Convierte un valor numérico a texto legal en dólares y centavos.

    Args:
        monto: Cantidad monetaria a convertir. Puede ser numérica o cadena
            convertible a ``float``.
        incluir_centavos: Si es ``True``, incluye la parte de centavos en el
            resultado. Actualmente siempre se incluye; el parámetro se mantiene
            por compatibilidad con futuras extensiones.

    Returns:
        Cadena en mayúsculas con el monto expresado en palabras, por ejemplo
        ``'DOCE DÓLARES CON CINCUENTA CENTAVOS'``.

    Example:
        >>> monto_a_letras(12.50)
        'DOCE DÓLARES CON CINCUENTA CENTAVOS'
        >>> monto_a_letras(1)
        'UN DÓLAR CON CERO CENTAVOS'
    """
    monto_str = f"{float(monto):.2f}"
    entero_str, decimal_str = monto_str.split(".")

    entero = int(entero_str)
    centavos = int(decimal_str)

    if entero == 1:
        texto_dolares = "UN DÓLAR"
    else:
        letras_entero = num2words(entero, lang="es").upper()
        if letras_entero.endswith("UNO"):
            letras_entero = letras_entero[:-1]  # Eliminamos la 'O'
        texto_dolares = f"{letras_entero} DÓLARES"

    if centavos == 0:
        texto_centavos = "CERO CENTAVOS"
    elif centavos == 1:
        texto_centavos = "UN CENTAVO"
    else:
        letras_decimal = num2words(centavos, lang="es").upper()
        if letras_decimal.endswith("UNO"):
            letras_decimal = letras_decimal[:-1]
        texto_centavos = f"{letras_decimal} CENTAVOS"

    return f"{texto_dolares} CON {texto_centavos}"
